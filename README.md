...
1. 개요 
	* 전역 상태 관리에 용이함
	* 리액트에 종속성 없는 라이브러리임. 바닐라 js에도 쓸수있음.

2. 키워드
	* 액션
		* 상태에 어떤 변화가 필요하면 액션이 발생.
		* JSON 형태이며 무조건 type 키를 가져야 함.
		* type이 액션의 이름이라고 생각하면 됨.
		* type외의 다른 키들은 완전히 자유.
	* 액션 생성 함수
		* 액션 객체를 만들어줌.
		* 액션 객체를 반환하면 됨. 변화를 일으킬때마다 액션객체를 만들어서 리턴하기 귀찮으니까 이렇게 씀.
	* 리듀서
		* 변화를 일으키는 함수
		*  '현재 상태', '액션'
		  을 인자로 받아서 => '새로운 상태' 를 리턴하는 형태
	* 스토어
		* 애플리케이션의 상태와 리듀서 및 몇가지 중요 내장함수를 포함함.
		* 프로젝트에 리덕스 적용하려면 필요. 한 개의 프로젝트는 단 하나의 스토어를 가짐.
	* 디스패치
		* 스토의 내장함수임. '액션을 발생시키는 것'.
		* dispatch(action) 형태로 액션객체를 인자로 넣어 호출.
		* 호출시 스토어는 리듀서 함수를 실행시키고 새로운 상태를 만듬.
	* 구독
		* 얘도 스토어의 내장함수임.
		* subscribe 함수 안에 리스너 함수를 파라미터로 넣어서 호출하면 해당 리스너에 액션이 디스패치됨. => 상태가 업데이트될때마다 호출됨
